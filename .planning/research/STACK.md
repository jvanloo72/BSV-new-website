# Stack Research — Belcher, Smolen & Van Loo LLP Website

**Domain:** Boutique law firm marketing website (content-driven, low-traffic, conversion-via-contact-form)
**Researched:** 2026-05-25
**Confidence:** HIGH — every version below was confirmed against the live npm registry and Astro/Vercel/Tailwind/Resend official documentation via Context7 on the research date. WebSearch and WebFetch were unavailable during research, so a few "ecosystem popularity" claims are MEDIUM; those are flagged inline.

> **Locked foundation (not in scope to revisit):** Astro 6 + Tailwind CSS v4 + GitHub + Vercel.
> This document recommends what to put *on top* of that foundation.
...
This is the single most consequential decision in the supporting stack and the one Jon explicitly flagged as TBD. The decision matters because of **ABA Formal Opinion 477R**: the firm has a professional responsibility to protect the confidentiality of pre-engagement client communications, including web-form submissions. That is *not* a casual constraint — it shapes which vendor is acceptable.

### Primary Recommendation: **Vercel serverless function (Astro Action) + Resend, email-only delivery, no database**

**The pattern:**
1. Contact form posts to an Astro server action (`src/actions/contact.ts`) with `accept: 'form'` and a Zod input schema (`name`, `email`, `phone`, `matterType`, `message`, `honeypot`).
2. The action runs server-side as a Vercel serverless function: validates input, checks the honeypot field is empty, rate-limits by IP (Upstash Redis free tier is overkill — start with an in-memory check), and calls Resend to send an email to `intake@bsvlaw.com`.
3. **No database, no row of customer data persisted at rest anywhere except the firm's existing email system** (Microsoft 365 / Google Workspace, whichever BSV uses).
4. The Resend API key lives in `Vercel → Settings → Environment Variables` as `RESEND_API_KEY`. Never in source, never in git.
5. Form sends a confirmation email back to the prospect ("We received your inquiry; a partner will respond within one business day. This email does not create an attorney-client relationship.") — also through Resend.

**Why this is right for BSV:**
...
**Costs:** Resend free tier is 3,000 emails/month / 100 emails/day. For a referral-driven boutique that gets a handful of inbound inquiries a week, this is permanently free. The Vercel Hobby tier covers the function invocations.

### Backup Recommendation (if Jon's firm wants leads in a database for follow-up tracking): **Vercel + Resend + Supabase**

Same flow as above, but the action also inserts the lead into a Supabase `leads` table before sending the email. This is only worth doing if:
...
**Critical additional security work if we go this route** (don't skip these — they're what `/hc-firm-site:check` will audit):
...
> **Does BSV currently use a CRM or intake system (e.g., Clio Grow, HubSpot, Salesforce)?**
>
> - If **no** → use the Primary Recommendation (Astro Action + Resend, email-only).
> - If **yes** → use the Primary Recommendation **plus** also POST the lead to the CRM's inbound webhook. (Belt-and-suspenders: email goes to the partners' inbox, CRM tracks the pipeline.)
> - If **the firm wants searchable lead history but no CRM** → Backup (add Supabase with RLS).

---

## Installation (Primary Recommendation)

These are the commands the GSD setup phase will run. Jon doesn't run any of this manually.
...
npm install @astrojs/vercel@10.0.7 @astrojs/mdx@5.0.6 @astrojs/sitemap@3.7.2 @astrojs/rss@4.0.18
...
# SEO / structured data
npm install schema-dts@1.1.0 astro-seo@1.0.0 astro-robots-txt@1.0.0

# Icons + images
npm install astro-icon@1.1.5 sharp@0.34.5
...
# Dev dependencies
npm install -D @playwright/test@1.60.0 @axe-core/playwright@4.11.3 @lhci/cli@0.15.1
npm install -D @astrojs/check@0.9.9 typescript@latest
npm install -D eslint eslint-plugin-astro@1.7.0 eslint-plugin-jsx-a11y@6.10.2
npm install -D prettier@3.8.3 prettier-plugin-astro@0.14.1 prettier-plugin-tailwindcss@0.8.0
npm install -D rehype-external-links@3.0.0
```
...
**If BSV has a CRM (Clio Grow / HubSpot / Salesforce):**
...
**If BSV later wants lead-history search but doesn't want a CRM:**
...
All confirmations performed 2026-05-25 against the live npm registry and Astro/Vercel/Tailwind/Resend documentation via Context7.
...
**WebSearch and WebFetch were unavailable during this research.** Where I noted MEDIUM confidence (specifically: `astro-seo` continued popularity in 2026 vs. hand-rolled `<head>`), that's because I couldn't survey 2026 community usage independently. The recommendation is still safe — `astro-seo` is small, MIT-licensed, and either way the cost of choosing wrong is one file's worth of refactor.

---

*Stack research for: boutique law firm marketing website (BSV Law)*
*Researched: 2026-05-25*
